...
impl Partial {
    pub const fn formula(self) -> Option<&'static str> {
        match self {
            Self::Direct => None,
...
macro_rules! ops {
    (@declare $code:expr;) => {};
    (@declare $code:expr; $family:ident $name:ident = $label:literal apply $apply:literal partials [$($partial:tt),*]; $($rest:tt)*) => {
        pub const $name: u32 = $code;
        ops!(@declare $code + 1u32; $($rest)*);
    };
    ($($family:ident $name:ident = $label:literal apply $apply:literal partials [$($partial:tt),*];)+) => {
...
pub fn declarations() -> String {
    ...
    for op in OPS {
        writeln!(out, "const {}: u32 = {}u;", op.constant, op.code).unwrap();
    }
    out
}
