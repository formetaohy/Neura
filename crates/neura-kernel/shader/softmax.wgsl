fn run_softmax(task: Task, lid: u32, slot: u32) {
    ...
    for (var row = task.first; row < task.first + task.count; row = row + 1u) {
        ...
        for (var column = lid; column < columns; column = column + WORKGROUP_SIZE) {
            ...
            publish(output, index, chained(task, row_at + vec4<u32>(0u, 0u, 0u, column), exp(fetch(source, row_origin(source, row) + column * source.strides.w) - row_max) / row_sum, slot));
        }
        workgroupBarrier();
    }
}

fn run_softmax_grad(task: Task, lid: u32, slot: u32) {
    ...
    for (var row = task.first; row < task.first + task.count; row = row + 1u) {
        ...
        for (var column = lid; column < columns; column = column + WORKGROUP_SIZE) {
            ...
            publish(output, index, chained(task, row_at + vec4<u32>(0u, 0u, 0u, column), y * (g - row_dot), slot));
        }
        workgroupBarrier();
    }
}

fn run_log_softmax(task: Task, lid: u32, slot: u32) {
    let source = values[task.a];
    let output = values[task.out];
    let columns = source.dims.w;
    for (var row = task.first; row < task.first + task.count; row = row + 1u) {
        ...
        for (var column = lid; column < columns; column = column + WORKGROUP_SIZE) {
            let index = row * output.dims.w + column;
            publish(output, index, chained(task, row_at + vec4<u32>(0u, 0u, 0u, column), fetch(source, row_origin(source, row) + column * source.strides.w) - row_max - normalizer, slot));
        }
        workgroupBarrier();
    }
}

fn run_log_softmax_grad(task: Task, lid: u32, slot: u32) {
    let probability = values[task.a];
    let gradient = values[task.b];
    let output = values[task.out];
    let columns = probability.dims.w;
    for (var row = task.first; row < task.first + task.count; row = row + 1u) {
        ...
        for (var column = lid; column < columns; column = column + WORKGROUP_SIZE) {
            ...
            let y = fetch(probability, row * columns + column);
            let g = fetch(gradient, row * columns + column);
            publish(output, index, chained(task, row_at + vec4<u32>(0u, 0u, 0u, column), g - exp(y) * row_total, slot));
        }
        workgroupBarrier();
    }
}
