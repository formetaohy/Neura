...
fn chained(task: Task, at: vec4<u32>, carried: f32, slot: u32) -> f32 {
    var result = carried;
...
fn run_binary(task: Task, lid: u32, slot: u32) {
    ...
    for (var index = task.first + lid; index < task.first + task.count; index = index + WORKGROUP_SIZE) {
        ...
        publish(output, index, chained(task, at, op_apply(task.kind, task.op, a, b, slot), slot));
    }
}

fn run_unary(task: Task, lid: u32, slot: u32) {
    ...
    for (var index = task.first + lid; index < task.first + task.count; index = index + WORKGROUP_SIZE) {
        ...
        publish(output, index, chained(task, at, op_apply(task.kind, task.op, a, 0.0, slot), slot));
    }
}

fn run_fill(task: Task, lid: u32, slot: u32) {
    let output = values[task.out];
    let dims = output.dims;
    for (var index = task.first + lid; index < task.first + task.count; index = index + WORKGROUP_SIZE) {
        publish(output, index, chained(task, coordinates(index, dims), task.param, slot));
    }
}

fn run_broadcast(task: Task, lid: u32, slot: u32) {
    let output = values[task.out];
    let source = values[task.a];
    let dims = output.dims;
    for (var index = task.first + lid; index < task.first + task.count; index = index + WORKGROUP_SIZE) {
        let at = coordinates(index, dims);
        publish(output, index, chained(task, at, fetch(source, read_address(at, source.strides)), slot));
    }
}
